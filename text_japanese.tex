...
\subsection{WebAssembly 命令リファレンス (抜粋)}

この資料は、授業でコンパイラを作成する際に最低限必要となる
WebAssembly (Wasm) の命令をまとめたものである。
Wasm には多くの命令が存在するが、ここでは整数演算と制御構造に関するものを中心に示す。

\subsubsection{基本的な命令}

\begin{itemize}
  \item \texttt{i32.const <n>} \quad 32ビット整数 \texttt{n} をスタックに積む。
  \item \texttt{local.get <idx>} \quad ローカル変数 \texttt{idx} をスタックに積む。
  \item \texttt{local.set <idx>} \quad スタックの先頭の値をローカル変数 \texttt{idx} に格納する。
  \item \texttt{local.tee <idx>} \quad 値をローカル変数に格納しつつ、そのままスタックに残す。
\end{itemize}

\subsubsection{算術演算命令}

\begin{itemize}
  \item \texttt{i32.add} \quad 2つの整数を加算する。
  \item \texttt{i32.sub} \quad 2つの整数を減算する。
  \item \texttt{i32.mul} \quad 2つの整数を乗算する。
  \item \texttt{i32.div\_s} \quad 符号付き整数の除算を行う。
\end{itemize}

\subsubsection{比較命令}

\begin{itemize}
  \item \texttt{i32.eq} \quad 等しいなら 1、そうでなければ 0 を返す。
  \item \texttt{i32.ne} \quad 等しくないなら 1、そうでなければ 0 を返す。
  \item \texttt{i32.lt\_s} \quad 左が右より小さいなら 1。
  \item \texttt{i32.gt\_s} \quad 左が右より大きいなら 1。
  \item \texttt{i32.le\_s} / \texttt{i32.ge\_s} \quad 以下 / 以上。
\end{itemize}

\subsubsection{制御構造}

\begin{itemize}
  \item \texttt{block \{ ... \}} \quad 命令列をブロックとしてまとめる。
  \item \texttt{loop \{ ... \}} \quad 繰り返し構造を作る。
  \item \texttt{br <n>} \quad 外側から数えて \texttt{n} 番目のブロックにジャンプする。
  \item \texttt{br\_if <n>} \quad スタックの先頭が非0なら \texttt{br <n>} と同じ動作。
  \item \texttt{if \{ ... \} else \{ ... \}} \quad 条件分岐。
\end{itemize}

\subsubsection{サンプル: 加算関数}

\begin{lstlisting}[language=lisp, caption={2つの整数を加算する関数}]
(module
  (func $add (param $x i32) (param $y i32) (result i32)
    local.get $x   ;; スタックにxを積む
    local.get $y   ;; スタックにyを積む
    i32.add)       ;; 加算して返す
  (export "add" (func $add)))
\end{lstlisting}

\subsubsection{サンプル: if文}
\begin{lstlisting}[language=lisp, caption={xとyを比較し、大きい方を返す関数}]
(module
...
\subsubsection{サンプル: whileループ}
\begin{lstlisting}[language=lisp, caption={0からnまでを加算するループ}]
(module
...
今回、 \verb|begin| -- \verb|end| は \verb|Block| 型にコンパイルするとします。
したがって、文の連続を \verb|Seq| 型で表し、\verb|Seq| 型を \verb|Block| 型で包めば実現できます。
\verb|Seq| 型は文と文の連続なので文 (statement, s) のペア
(\verb|Seq of s * s|) で表現し、\verb|Block| 型は、文の連続を表す
\verb|Seq| を要素に\verb|Block of s| と定義すればよいでしょう。

そもそも、文の連続 \verb|Seq| はどのように表現すればよいでしょうか。
文の連続は、パーサジェネレータのレベルで補足することができます。具体的には、
\verb|parser.mly| に文の連続をパースできるルールを追加すれば実現できます。以下のocamlyacc のテク
ニックを用い、連続した文を \verb|Seq| 型へ変換します。
...
このとき、\verb|statments| ルールは次のように読みます。まず、\verb|statements| にマッチしたとき、一文
...
\subsection{3日目: 2日目の続き + WebAssemblyコンパイラの作成}
...
ラッグ・アンド・ドロップするか、ボタンから読み込ませます。実行に成功すると、何ら
かの答えが返ってきます。

\fbox{\textbf{実装課題}}では、 \verb|emit_wasm.ml| の実装済みのコードを参考に、
未実装の演算や文の WebAssembly 命令への翻訳を行ってください。
\verb|emit_wasm.ml| は簡単な演算や文ならコンパイルできますが、相変わらず演算など
が実装されていないので、それらを実装してください。実装した内容をレポートにまとめ
て提出してください。
